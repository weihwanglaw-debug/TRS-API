...
[ApiController, Route("api/fixtures"), Authorize]
public class FixturesController : ControllerBase
{
    ...
    public async Task<IActionResult> Get(int eventId, int programId)
    {
        ...
        return Ok(new { eventId, programId, f.FixtureMode, f.FixtureFormat, f.IsLocked, f.Phase,
                        bracketStateJson = f.BracketStateJson, f.UpdatedAt });
    }
    ...
    public async Task<IActionResult> Save(int eventId, int programId, [FromBody] SaveFixtureRequest req)
    {
        var f = await _db.Fixtures.FirstOrDefaultAsync(x => x.EventId == eventId && x.ProgramId == programId);
        if (f == null) {
            f = new Fixture { EventId = eventId, ProgramId = programId, CreatedAt = DateTime.UtcNow,
